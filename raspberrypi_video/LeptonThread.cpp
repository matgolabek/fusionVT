	...
	while(true) {
		...
		for(int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
			...
			for(int i=0;i<FRAME_SIZE_UINT16;i++) {
				...
				//
				value = (valueFrameBuffer - minValue) * scale;  // T w RGBT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				...
				color = qRgb(colormap[ofs_r], colormap[ofs_g], colormap[ofs_b]);  //kolor piksela
				if (typeLepton == 3) {
		...
		//myImage = myImage.scaled(640, 480, Qt::KeepAspectRatio, Qt::SmoothTransformation);
